---
description: Bump the app version and trigger the Build APK release workflow
...
builds the release APK and publishes it as a GitHub Release. This command does
...
- `git status` — the working tree must be clean. If it isn't, stop and ask the
  user to commit or stash first; this command does not touch unrelated changes.
...
**If no argument was given**, use `AskUserQuestion` to ask which part to bump.
...
command — running `/release` is the user's confirmation — but if the push is
...
finishes; this command's job ends once the run is confirmed started.
